...
from zk.visual import Visual, VisualType
...
def listdir_oribyte(path):
    ...
    if os.path.exists(path):
        ...
        byte_samples = os.listdir(path)
    else:
        log.info('{} not exit'.format(path))
        return sample_path

    for item in byte_samples:
...
def do_make(file_path, label, visualer):
    ...
    byte_id = os.path.basename(file_path).split('.')[0]
    log.info('parse byte_id = {}'.format(byte_id))
    byte_label = label[byte_id]
    log.info('{} is {}'.format(byte_id, byte_label))
    
    try:
        byte_np, byte_img = visualer(file_path)
    except Exception as e:
        log.error(e)
        return False
...
async def dispatch_work(executor, sample_path):
    ...
    visualer = Visual(vtype=VisualType.BYTES, vwidth=VENCODELEN, vsize=VSIZE)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        datefmt='%m-%d %H:%M',
                        filename=('mcc2015_maker_real_nara.log'),
                        filemode='w')
